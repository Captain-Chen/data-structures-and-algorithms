...
@pytest.fixture
def hash_table():
    sample_data = MyHashTable(capacity=100)
...
def test_should_report_length_of_empty_hashtable():
    assert len(MyHashTable(capacity=100)) == 0

def test_should_report_capacity_of_empty_hashtable():
    assert MyHashTable(capacity=100).capacity == 100
...
def test_should_insert_none_value():
    hash_table = MyHashTable(capacity=100)
    hash_table["key"] = None
...
def test_should_not_return_empty_pairs(hash_table):
